...
lexicon_word = open("data/lexicon_word_wlp.txt", "w")
lexicon_label = open("data/lexicon_label_wlp.txt", "w")

training_set = open("data/training_set_wlp", "w")
validation_set = open("data/validation_set_wlp", "w")
testing_set = open("data/testing_set_wlp", "w")
...
for x,y in zip(split_t, split_f):
    if x != '' and y != '':
        ...
        wpl =str(a)+"$"+str(d)+"$"+str(e)
        words_train.append(wpl)
        iob.append(b)
        line_train.append((wpl, b))
    else:
...
for x,y in zip(split_test_t, split_test_f):
    if x != '' and y != '':
        ...
        c, d, e = y.split('\t') #word pos lemma
        wpl =str(a)+"$"+str(d)+"$"+str(e)
        words_test.append(wpl)
        iob.append(b)
        line_test.append((wpl, b))
    else:
